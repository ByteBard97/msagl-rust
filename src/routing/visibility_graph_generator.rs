...
fn process_open_vertex(
    ...
    create_scan_segments_from_neighbors(site, &low_neighbors, seg_tree, scan_direction);
    ...
    if need_high {
        create_scan_segments_from_neighbors(site, &high_neighbors, seg_tree, scan_direction);
    }
...
/// Creates up to 3 segments: Normal, Overlapped, Normal.
fn create_scan_segments_from_neighbors(
    ...
    if low_overlap.is_none() && high_overlap.is_none() {
        // No overlaps: single normal segment
        add_segment(
