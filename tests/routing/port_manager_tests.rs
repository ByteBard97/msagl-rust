...
fn splice_adds_edges_to_aligned_vertices() {
    ...
    let splice = PortManager::splice_port(&mut session, Point::new(5.0, 0.0));
    let port_id = splice.port_vertex;

    // Should have added edges to v1 (west, dist=5) and v2 (east, dist=5)
    assert!(session.vis_graph.out_degree(port_id) > 0);
}
...
fn unsplice_removes_added_edges() {
    ...
    let mut splice = PortManager::splice_port(&mut session, Point::new(5.0, 0.0));
    let port_id = splice.port_vertex;

    assert!(session.vis_graph.out_degree(port_id) > 0);

    PortManager::unsplice(&mut session, &mut splice);
    assert_eq!(session.vis_graph.out_degree(port_id), 0);
}
...
fn splice_and_route_between_obstacles() {
    ...
    let mut session = generate_visibility_graph(&shapes, 2.0);
    ...
    let mut src_splice = PortManager::splice_port(&mut session, source);
    let mut tgt_splice = PortManager::splice_port(&mut session, target);

    let search = PathSearch::new(4.0);
    let path = search.find_path(&mut session.vis_graph, source, target);
